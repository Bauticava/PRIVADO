...
from entity_spawner.srv import LoadEntity, SpawnEntity
...
class EntitySpawnerMainWindow(QMainWindow):
    ...
    def __init__(self, node: EntitySpawnerClientNode) -> None:
        ...
        spawn_layout.addWidget(spawn_button, 5, 0, 1, 3)
        ...
        main_layout.addWidget(self._status_label, 2, 0)
    ...
    def _on_spawn_clicked(self) -> None:
        ...
        future = self._node.spawn_entity(entity_name, frame_id, position, orientation, scale_value)
        self._enqueue_future(future, partial(self._handle_service_response, action='Spawn'))
        self._set_status(
            f"Spawning entity '{entity_name}' in frame '{frame_id}' with scale {scale_value:.3f}..."
        )
    ...
    def _spin_ros(self) -> None:
        ...
        if self._node.load_service_ready() and self._node.spawn_service_ready():
